...
func (g *Gwi) Private(h http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok || user != g.config.GitUser || pass != gitPass {
...
func (g *Gwi) GitCGIHandler(w http.ResponseWriter, r *http.Request) {
	...
	env := []string{
		"GIT_PROJECT_ROOT=" + g.config.GitRoot,
		"GIT_HTTP_EXPORT_ALL=1",
		"REMOTE_USER="+g.config.GitUser,
	}

	logger.Debug("using root: ", g.config.CGIPrefix)
	logger.Debug("cgiPath: ", g.config.GitCGIRoot)
	logger.Debug("using env: ", env)
	handler := &cgi.Handler{
		Path:   g.config.GitCGIRoot,
